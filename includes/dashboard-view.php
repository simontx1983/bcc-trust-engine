<?php
/**
 * Trust Engine Dashboard View
 *
 * Contains all tab rendering functions for the admin dashboard.
 *
...
function bcc_trust_render_all_pages_tab() {
    ...
    // SQL query - your query is perfect
    $sql = "SELECT 
                p.ID,
                p.ID as page_id,
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($pages as $page): 
                        ...
                        // Get the correct owner - USE $page->ID here
                        $correct_owner_id = bcc_trust_get_page_owner($page->ID);
                        if (!$correct_owner_id || $correct_owner_id == 0) {
                            $correct_owner_id = $page->post_author ?? 0;
                        }
                        
                        $has_owner = $correct_owner_id > 0;
                        $owner_display = '—';
                        $owner_link = '#';
                        
                        if ($has_owner) {
                            $user = get_userdata($correct_owner_id);
                            if ($user) {
                                $owner_display = $user->display_name ?: $user->user_login;
                                $owner_link = admin_url('admin.php?page=bcc-trust-moderation&user_id=' . $correct_owner_id);
                            } else {
                                $owner_display = 'User #' . $correct_owner_id;
                            }
                        }
                    ?>
                        <tr>
                            <td>
                                <strong>#<?php echo $page->ID; ?></strong> <!-- USE $page->ID HERE -->
                            </td>
                            <td>
                            ...
                            <td>
                                <?php if ($has_owner): ?>
                                    <a href="<?php echo esc_url($owner_link); ?>">
                                        <?php echo esc_html($owner_display); ?>
                                    </a>
                                    <br><small>ID: <?php echo $correct_owner_id; ?></small>
                                <?php else: ?>
                            ...
                            <td>
                                <a href="<?php echo get_permalink($page->ID); ?>" class="button button-small" target="_blank">View</a>
                                <?php if ($has_owner): ?>
                                    <a href="<?php echo esc_url($owner_link); ?>" class="button button-small">Owner</a>
                                <?php endif; ?>
                                <?php if (!$has_score): ?>
                                    <button class="button button-small" onclick="initializePageScore(<?php echo $page->ID; ?>)">Init</button>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
